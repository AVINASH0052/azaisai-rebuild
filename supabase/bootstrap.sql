...
-- Sign-in asks "does this email exist?" before the password field.
create or replace function public.email_registered(addr text)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where email is not null
      and lower(email) = lower(trim(addr))
  );
$$;
