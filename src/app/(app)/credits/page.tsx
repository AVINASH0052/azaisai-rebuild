...
export default async function CreditsPage() {
  ...
  return (
    <main className="px-6 py-10">
      <h1 className="text-2xl text-fg">Credits</h1>
      <p className="mt-2 font-mono text-3xl">{balance}</p>
      <p className="mt-1 text-sm text-fg-muted">Running balance from the ledger.</p>
      <ul className="mt-8 divide-y divide-border">
        {(rows ?? []).map((row) => (
          <li key={row.id} className="flex items-center justify-between py-3 text-sm">
            <span className="text-fg-muted">{row.reason.replaceAll("_", " ")}</span>
